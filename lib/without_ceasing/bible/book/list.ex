defmodule WithoutCeasing.Bible.Book.List do
  ...
  @books %{
    ...
    "daniel" => %Book{
      name: "Daniel",
      testament: :old,
      chapters: [21, 49, 100, 34, 30, 29, 28, 27, 27, 21, 45, 13, 64, 42]
    },
    "hosea" => %Book{
      name: "Hosea",
      testament: :old,
    ...
    "thessalonians-1" => %Book{
      name: "1 Thessalonians",
      testament: :new,
      chapters: [10, 20, 13, 18, 28]
    },
    "thessalonians-2" => %Book{
  ...
  @doc """
    iex> Book.find("genesis")
    %Book{name: "Genesis", testament: :old, chapters: [...]}
  """
  def find(slug), do: do_lookup(slug)

  for {slug, book} <- @books do
    defp do_lookup(unquote(slug)), do: unquote(Macro.escape(book))
  end

  defp do_lookup(slug), do: raise(ArgumentError, "No books of the bible match #{slug}")

  def map(fun) do
    Enum.map(@order, fn book -> fun.({book, find(book)}) end)
  end

  def map_by_verse(fun) do
    Enum.flat_map(@order, fn book ->
      book_data = find(book)

      book_data.chapters
      |> Enum.with_index(&{&2 + 1, &1})
      |> Enum.reduce([], fn {chapter, verse_count}, acc ->
        acc ++ Enum.map(1..verse_count, &fun.({book, chapter, &1}))
      end)
    end)
  end
end
