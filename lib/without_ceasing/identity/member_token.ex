defmodule WithoutCeasing.Identity.MemberToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  def build_session_token(member) do
    ...
    {token,
     %WithoutCeasing.Identity.MemberToken{token: token, context: "session", member_id: member.id}}
  end
  ...
  defp build_hashed_token(member, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %WithoutCeasing.Identity.MemberToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       member_id: member.id
  ...
  def token_and_context_query(token, context) do
    from WithoutCeasing.Identity.MemberToken, where: [token: ^token, context: ^context]
  end
  ...
  def member_and_contexts_query(member, :all) do
    from t in WithoutCeasing.Identity.MemberToken, where: t.member_id == ^member.id
  end

  def member_and_contexts_query(member, [_ | _] = contexts) do
    from t in WithoutCeasing.Identity.MemberToken,
