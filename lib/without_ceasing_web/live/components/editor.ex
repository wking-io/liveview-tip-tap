defmodule WithoutCeasingWeb.Components.Editor do
  ...
  def editor(assigns) do
    ~H"""
      <div id="editor-root" class="h-full flex flex-col" phx-hook="Editor" data-content={@content}>
        <div class="border-b border-gray-900 flex-1">
        ...
        <div id="editor-element" phx-ref="element" class="h-full"></div>
        </div>
        <.form
          let={f}
          for={@changeset}
  ...
  def editor_button(assigns) do
    ~H"""
    <button
      class={"h-full w-[38px] flex items-center justify-center hover:bg-gray-200 focus:bg-gray-200 #{focus_classes()}"}
  ...
  def dropdown_button(assigns) do
    ~H"""
    <button
      class={"flex items-center w-full text-left py-2 px-4 hover:bg-gray-300 focus:bg-gray-300 #{focus_classes()}"}
      phx-click={editor_action(@action)}
    >
      <%= if assigns[:icon] do %>
        <svg
  ...
  defp editor_action(:heading_five) do
    JS.dispatch("editor-button:action", detail: %{action: "toggleHeading", level: 5})
  end

  defp editor_action(:heading_six) do
    JS.dispatch("editor-button:action", detail: %{action: "toggleHeading", level: 6})
  end
